...
BEGIN   
    ...
    BEGIN TRY
        ...
        FROM RFM_Diem rfm
        ORDER BY N'Phân loại khách hàng' DESC
    END TRY
    BEGIN CATCH
        PRINT 'Lỗi: ' + ERROR_MESSAGE()
    END CATCH
...
BEGIN
    ...
    BEGIN TRY
        ...
        VALUES (@NgayLap, @KhuyenMai, NULL, @MaCN, @MaKH, N'Chưa thanh toán', @MaNV);
...
BEGIN
    ...
    BEGIN TRY
        ...
        -- Hóa đơn phải còn trạng thái 'Chưa thanh toán'
        IF NOT EXISTS (
            SELECT 1
            FROM HOADON
            WHERE MAHD = @MaHD
...
END;
go
CREATE PROCEDURE sp_ChiNhanh_DanhSach
AS
BEGIN
    SET NOCOUNT ON;
...
CREATE PROCEDURE sp_TraCuu_BacSi
    @MaCN INT = NULL,
...
CREATE PROCEDURE sp_SanPham_TimKiemOnline
...
CREATE PROCEDURE sp_DonHangOnline_Tao
    @MaKH INT,
    @TongTien DECIMAL(12, 2),
    @MaHD INT OUTPUT
AS
...
CREATE PROCEDURE sp_DonHangOnline_ThemChiTiet
    @MaHD INT,
    @MaSP INT,
    @SoLuong INT
AS
...
CREATE PROCEDURE sp_NhanVien_TraCuuThuCungToanHeThong
    @Keyword NVARCHAR(100) = NULL,
    @FilterType NVARCHAR(20) = N'Tất cả'
AS
BEGIN
    SET NOCOUNT ON;
    ...
    WITH CustomerStatus AS (
        SELECT 
            ...
            KH.TENCAPBAC,
            -- Đếm lượt khám tiêm
            (SELECT COUNT(*) FROM CAKHAMBENH WHERE MATC = TC.MATC) AS SoLanKham,
            (SELECT COUNT(*) FROM CATIEM WHERE MATC = TC.MATC) AS SoLanTiem,
            -- Đếm hóa đơn mua hàng
            (SELECT COUNT(*) FROM HOADON WHERE MAKH = KH.MAKH AND TRANGTHAI = N'Đã thanh toán') AS SoHoaDon
        FROM THUCUNG TC
        INNER JOIN KHACHHANG KH ON TC.MAKH = KH.MAKH
        WHERE (@Keyword IS NULL OR @Keyword = ''
            OR TC.TENTC LIKE N'%' + @Keyword + N'%'
            OR KH.TENKH LIKE N'%' + @Keyword + N'%'
            OR KH.SDT LIKE '%' + @Keyword + '%')
    ),
    FinalSelection AS (
        SELECT 
            MATC, TENTC, LOAI, GIOITINH, TINHTRANG, TENKH, SDT, TENCAPBAC,
            -- Logic: Nếu có lượt khám/tiêm hoặc hóa đơn thì là Khách cũ
            CASE 
                WHEN (SoLanKham + SoLanTiem + SoHoaDon) > 0 THEN N'Khách cũ'
                ELSE N'Khách mới'
            END AS [LoaiKhach],
            (SoLanKham + SoLanTiem) AS [TongLuotKhamTiem]
        FROM CustomerStatus
    )
    SELECT * FROM FinalSelection
    WHERE 
        (@FilterType = N'Tất cả')
        OR (@FilterType = N'Khách cũ' AND [LoaiKhach] = N'Khách cũ')
        OR (@FilterType = N'Khách mới' AND [LoaiKhach] = N'Khách mới')
        OR (@FilterType = N'Đã từng khám' AND [TongLuotKhamTiem] > 0)
    ORDER BY MATC DESC;
END;
GO
...
BEGIN
    ...
    SELECT 
        ...
        T.DONVI,
        S.DONGIA,
        -- Tạo một chuỗi hiển thị kết hợp để bác sĩ dễ nhìn
        S.TENSP + ' (' + T.DONVI + ') - Tồn: ' + CAST(S.TONKHO AS NVARCHAR(10)) AS Display
    FROM SANPHAM S
    JOIN THUOC T ON S.MASP = T.MATHUOC
    WHERE S.LOAI = 'Th' AND S.TONKHO > 0
...
-- CREATE OR ALTER PROCEDURE sp_BacSi_LayDanhSachCho
--     @MaBS INT
-- AS
-- BEGIN
--     SELECT 
...
--     JOIN KHACHHANG KH ON TC.MAKH = KH.MAKH
--     WHERE KB.MANV = @MaBS 
--       AND NOT EXISTS (SELECT 1 FROM TOATHUOC WHERE MAKB = KB.MAKB) -- Chưa có toa thuốc là đang khám/chờ
--     ORDER BY KB.NGAYKHAM DESC;
-- END;
-- GO
-- CREATE OR ALTER PROCEDURE sp_Thuoc_TraCuuGiaoDien
--     @Keyword NVARCHAR(100) = NULL
-- AS
-- BEGIN
--     SET NOCOUNT ON;
...
--         T.DONVI AS [Đơn Vị], 
--         T.NGAYSX AS [Ngày Sản Xuất],
--         T.HSD AS [Hạn Dùng (Tháng)],
--         S.DONGIA AS [Giá Bán], 
--         S.TONKHO AS [Tồn Kho]
--     FROM SANPHAM S
--     JOIN THUOC T ON S.MASP = T.MATHUOC
--     WHERE S.LOAI = 'Th'
--       AND (@Keyword IS NULL OR S.TENSP LIKE N'%' + @Keyword + N'%')
--     ORDER BY S.TONKHO DESC; -- Ưu tiên thuốc còn nhiều hàng lên đầu
-- END;
-- GO
CREATE PROCEDURE sp_BacSi_LayDanhSachBenhNhanCho
    @MaBS INT
AS
BEGIN
    SET NOCOUNT ON;
    ...
    SELECT 
        ...
        KH.MAKH,
        TC.MATC,
        -- Hiển thị tên khách + tên thú cưng + giờ để dễ chọn
        KH.TENKH + N' - Pet: ' + TC.TENTC + N' (' + TC.LOAI + N')' AS Display,
        KH.TENKH,
        KH.SDT,
        TC.TENTC,
        KB.NGAYKHAM
    ...
    JOIN KHACHHANG KH ON TC.MAKH = KH.MAKH
    WHERE KB.MANV = @MaBS
      -- Chỉ lấy lịch của hôm nay
      AND CAST(KB.NGAYKHAM AS DATE) = CAST(GETDATE() AS DATE)
      -- Chỉ lấy ca chưa hoàn thành (ví dụ: chưa có toa thuốc)
      AND NOT EXISTS (SELECT 1 FROM TOATHUOC TT WHERE TT.MAKB = KB.MAKB)
    ORDER BY KB.NGAYKHAM ASC;
END;
GO
CREATE PROCEDURE sp_ChiNhanh_LayDanhSachBacSi
    @MaCN INT
AS
BEGIN
    ...
    JOIN LAMVIEC LV ON NV.MANV = LV.MANV
    WHERE LV.MACN = @MaCN 
      AND LV.VAITRO = 'BS' -- Chỉ lấy bác sĩ
      AND (LV.NGAYKETTHUC IS NULL OR LV.NGAYKETTHUC >= CAST(GETDATE() AS DATE))
END
GO
CREATE PROCEDURE sp_NhanVien_TimKhachHang
    @TuKhoa NVARCHAR(100)
AS
BEGIN
    SELECT MAKH, TENKH, SDT, DIACHI
    FROM KHACHHANG
    WHERE TENKH LIKE N'%' + @TuKhoa + N'%'
       OR SDT LIKE '%' + @TuKhoa + '%'
    ORDER BY TENKH ASC;
END;